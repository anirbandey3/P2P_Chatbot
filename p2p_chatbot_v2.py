    ...
    if st.session_state["doc_data"] is not None:
        ...
        if query := st.chat_input("Ask me about Payables..."):
            ...
            with st.spinner("Analyzing documentation and web sources..."):
                # First, look for schema-qualified table names (Schema.TableName pattern)
                ...
                if schema_table_match:
                    # If we found a pattern like "GL.GL_ACCESS_SETS"
                    ...
                    print(f"Detected schema.table pattern: {schema_name}.{table_name}")
                    response = fetch_table_columns_from_json(table_name, schema_name=schema_name)
                else:
                    # Check if the query is about a specific table
                    table_pattern = r'\b([A-Z][A-Z0-9_]{3,}(?:_ALL|_INTERFACE|_HEADERS|_LINES|_DETAILS|_ACCOUNTS|_SETS|_PERIODS|_BALANCES|_VALUES|_TYPES)?)\b'
                    table_match = re.search(table_pattern, query.upper())
                    
                    if table_match:
                        table_name = table_match.group(1)
                        print(f"Detected table: {table_name}")
                        
                        # Store detected table
                        st.session_state.detected_table = table_name
                        
                        # Look for schema mentioned in the query
                        ...
                        # Debug prefix check
                        print(f"Table: {table_name}, starts with GL_? {table_name.startswith('GL_')}")
                        
                        # Determine schema - if found in query use that, otherwise default to AP
                        schema_name = schema_match.group(1) if schema_match else "AP"
                        
                        # Special handling for common tables prefixes
                        if table_name.startswith("GL_") and not schema_match:
                            schema_name = "GL"
                            print("Setting schema to GL based on table prefix")
                        elif table_name.startswith("AP_") and not schema_match:
                            schema_name = "AP"
                        elif table_name.startswith("AR_") and not schema_match:
                            schema_name = "AR"
                        elif table_name.startswith("PO_") and not schema_match:
                            schema_name = "PO"
                        ...
                        print(f"FINAL: Table={table_name}, Schema={schema_name}")
                        
                        # Call function with detected schema
                        response = fetch_table_columns_from_json(table_name, schema_name=schema_name)
                    else:
                        # Get doc content
                ...
                if response:
                    ...
                    display_chat_message("assistant", response, message_id=message_id)
